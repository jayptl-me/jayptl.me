...
test('every content route negotiates to markdown', async () => {
  for (const route of ['/about', '/resume', '/privacy', '/projects',
...
test('direct .md URLs are served with text/markdown', async () => {
  for (const p of ['/index.md', '/about.md', '/resume.md', '/projects.md', '/projects/aviz-health.md']) {
...
test('www host redirects 301 to apex canonical', async () => {
  const res = await get('/about', { 'X-Forwarded-Host': 'www.jayptl.me' });
  assert.equal(res.status, 301);
  assert.equal(res.headers.get('location'), 'https://jayptl.me/about');
});

test('legacy .html and direct /pages/ paths 301-redirect to clean canonical URLs', async () => {
  const redirects = [
    ['/index.html', '/'],
    ['/about.html', '/about'],
    ['/resume.html', '/resume'],
  ...
  for (const [source, destination] of redirects) {
    ...
    assert.equal(res.status, 301, `${source} must return 301`);
    assert.equal(res.headers.get('location'), destination, `${source} location header must be ${destination}`);
  }
});

test('legacy redirects preserve query strings', async () => {
  const res = await get('/about.html?ref=google');
  assert.equal(res.status, 301);
  assert.equal(res.headers.get('location'), '/about?ref=google');
});

test('www host with legacy path redirects directly to apex canonical path', async () => {
  const res = await get('/about.html?ref=twitter', { 'X-Forwarded-Host': 'www.jayptl.me' });
  assert.equal(res.status, 301);
  assert.equal(res.headers.get('location'), 'https://jayptl.me/about?ref=twitter');
});
