...
const CACHE_CONTROL = [
  // Versioned resumes change per application; keep shared cache short.
  // Must precede /assets/ (first match wins).
  { prefix: '/assets/resumes/', value: 'public, max-age=3600, must-revalidate' },
...
const PERMANENT_REDIRECTS = {
  ...
  '/pages/about': '/about',
  '/pages/resume.html': '/resume',
  '/pages/resume': '/resume',
  '/pages/privacy.html': '/privacy',
  '/pages/privacy': '/privacy',
  '/pages/design-system.html': '/design-system',
  '/pages/design-system': '/design-system',
  '/pages/projects/index.html': '/projects',
  '/pages/projects/index': '/projects',
  '/pages/projects.html': '/projects',
  '/pages/projects': '/projects',
  '/pages/projects/aviz-health.html': '/projects/aviz-health',
  '/pages/projects/aviz-health': '/projects/aviz-health',
  '/pages/projects/swalook.html': '/projects/swalook',
  '/pages/projects/swalook': '/projects/swalook',
  '/pages/projects/genuinest.html': '/projects/genuinest',
  '/pages/projects/genuinest': '/projects/genuinest',
  '/pages/projects/vini-tini.html': '/projects/vini-tini',
  '/pages/projects/vini-tini': '/projects/vini-tini',
  '/pages/about.md': '/about.md',
  '/pages/resume.md': '/resume.md',
  '/pages/privacy.md': '/privacy.md',
  '/pages/projects/index.md': '/projects.md',
...
function getCanonicalRedirect(pathname) {
  ...
  // Generic /pages/ prefix handler: prevent internal path leaks
  if (pathname.startsWith('/pages/')) {
    const stripped = pathname.slice('/pages'.length);
    const clean = stripped.replace(/\/index\.(html|md)$/, '').replace(/\.(html|md)$/, '') || '/';
    if (ROUTES[clean] || MD_ROUTES[clean] || clean === '/') {
      return clean;
    }
  }
  return null;
...
function createApp(options = {}) {
  ...
  function serveFile(req, res, filePath, requestPath, extraHeaders = {}) {
    fs.readFile(filePath, (err, data) => {
      ...
      // Phased R-B: versioned PDFs are attachments of the canonical /resume
      // until a role has distinct content + title. Per Google canonical docs,
      // sitemap alone is a weak signal, so send an explicit HTTP canonical.
      // X-Robots-Tag (not meta robots) is the only noindex that works for PDFs.
      const pdfHeaders = {};
      if (requestPath.endsWith('.pdf') && (requestPath.startsWith('/resumes/') || requestPath.startsWith('/assets/resumes/'))) {
        pdfHeaders.Link = '<https://jayptl.me/resume>; rel="canonical"';
        pdfHeaders['X-Robots-Tag'] = 'noindex';
      }
      const headers = baseHeaders(requestPath, {
  ...
  const server = http.createServer((req, res) => {
    ...
    // Canonical host defense: 301 redirect any www.* host to apex https://jayptl.me
    const host = (req.headers['x-forwarded-host'] || req.headers.host || '').split(':')[0].toLowerCase();
    if (host.startsWith('www.')) {
      const canonicalHost = host.replace(/^www\./, '');
      const redirectPath = getCanonicalRedirect(requestPath) || requestPath;
      const query = req.url.includes('?') ? req.url.slice(req.url.indexOf('?')) : '';
      res.writeHead(301, {
        'Location': `https://${canonicalHost}${redirectPath}${query}`,
    ...
    // Canonical path defense: 301 redirect legacy .html and direct /pages/ paths to clean URLs
    const targetRedirect = getCanonicalRedirect(requestPath);
    if (targetRedirect && targetRedirect !== requestPath) {
      const query = req.url.includes('?') ? req.url.slice(req.url.indexOf('?')) : '';
      res.writeHead(301, {
        'Location': `${targetRedirect}${query}`,
        'Cache-Control': 'public, max-age=31536000'
      });
      res.end();
      return;
    ...
    } else if (requestPath.startsWith('/resumes/') && requestPath.endsWith('.pdf')) {
      // Stable public URLs (/resumes/<role>.pdf) served from versioned assets.
      // Keeps LinkedIn/portal links stable while files rotate underneath.
